from cellophane.testing import BaseTest, Invocation, regex


class Test_tutorial(BaseTest):
    structure = {
    ...
    def test_tutorial(self, invocation: Invocation) -> None:
        assert invocation.exit_code == 0
        assert invocation.logs == regex(
            r"Running Some hook label hook",
            r"Sample b has a file larger than 6 bytes",
            r"Sample a processed successfully",
            r"Running my_post_hook_b hook",
            r"Setting my_attribute for sample a",
            r"Running my_post_hook_a hook",
            r"Woah! A message from a: Hello from a!",
        )
